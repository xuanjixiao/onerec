...
# classes
class ViT(nn.Module):
    def __init__(self, config, num_patches=4, classes=2):
        super(ViT, self).__init__()
        # order: seller/src_city/other(post)/dst_city
        self.num_patches = num_patches        # order length
        self.pool = 'cls' if config['pool'] is None else config['pool']           # order aggregation
        self.patch_dim = 128                  # order total embedding
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(self.dim, classes)      # pos/neg
        )
    ...
    def calculate_loss(self, orders, y):
        y_pred = self.forward(orders.to(self.device))
        return self.loss(torch.squeeze(y_pred), y.to(self.device))

    def predict(self, orders):
        return torch.squeeze(self.forward(orders.to(self.device)))
