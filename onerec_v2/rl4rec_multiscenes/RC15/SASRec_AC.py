    ...
    def __init__(self, hidden_size,learning_rate,item_num,state_size, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.output2 = tf.contrib.layers.fully_connected(self.state_hidden, self.item_num,
                                                             activation_fn=None, scope="ce-logits")  # all logits（不表示实际的概率）

            # TRFL way
            self.actions = tf.placeholder(tf.int32, [None])
            self.targetQs_ = tf.placeholder(tf.float32, [None, item_num])
            self.targetQs_selector = tf.placeholder(tf.float32, [None,item_num])  # used for select best action for double q learning
            ...
            # 对于self.actions中的每个索引，从self.output1中选择对应的Q值，形成一个新的形状为[batch_size]的张量，其中每个元素是对应动作的Q值。
            ...
            # 计算交叉熵损失（与celoss1相同），然后将其与Q值（q_indexed）相乘
            # 意味着Q值较高的样本在总损失中会有更大的影响
            celoss2 = tf.multiply(q_indexed, tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions,logits=self.output2)) # 
...
def evaluate(sess):
    ...
    while evaluated<len(eval_ids):
        ...
        for i in range(batch):
            ...
            evaluated+=1
        prediction=sess.run(SASRec1.output2, 
                            feed_dict={
                                SASRec1.inputs: states,
                                SASRec1.len_state:len_states,
                                SASRec1.is_training:False
                                })
        sorted_list = np.argsort(prediction) # 返回排序后的索引
...
if __name__ == '__main__':
    ...
    SASRec2 = SASRecnetwork(hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,state_size=state_size, name='SASRec2')
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                # 使用目标网络生成下一个状态的Q值：目标网络target_QN的一个输出操作（output1）
                ...
                # TODO 为什么卡3000步？
                if total_step<30000:
                    # 如果总步数小于30000，则使用不 带q-value加权的 combine loss
